...
const StudentTokenScreen = ({ navigation }) => {
  ...
  const GetStudentTokenByStudentId = async () => {
    try {
      ...
      const response = await httpGet(`StudentToken/getStudentTokenByStudentId?StudentId=${studentId.data.studentId}`);
  ...
  const handleAddStudentTokenNavigate = async () => {
    ...
    navigation.navigate('StudentTokenFormScreen', { studentId: studentIds.data.studentId })
  };
  ...
  const renderTokenCard = ({ item }) => (
    <View style={{
      ...
      <View style={{ flexDirection: 'row' }}>
        ...
        <Text style={{ fontSize: 16, fontWeight: 'bold', marginBottom: 8 }}>xxxx -/Rs</Text>
      </View>
      <View style={{ flexDirection: 'row' }}>
